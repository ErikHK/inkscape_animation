    ...
    new_window(NULL)
{
    ...
    xml_element_new_button.signal_clicked().connect(sigc::mem_fun(*this, &AnimationDialog::cmd_new_element_node));
    xml_text_new_button.signal_clicked().connect(sigc::mem_fun(*this, &AnimationDialog::cmd_new_text_node));
    xml_node_duplicate_button.signal_clicked().connect(sigc::mem_fun(*this, &AnimationDialog::cmd_duplicate_node));
    xml_node_delete_button.signal_clicked().connect(sigc::mem_fun(*this, &AnimationDialog::cmd_delete_node));
    unindent_node_button.signal_clicked().connect(sigc::mem_fun(*this, &AnimationDialog::cmd_unindent_node));
    indent_node_button.signal_clicked().connect(sigc::mem_fun(*this, &AnimationDialog::cmd_indent_node));
    raise_node_button.signal_clicked().connect(sigc::mem_fun(*this, &AnimationDialog::cmd_raise_node));
    lower_node_button.signal_clicked().connect(sigc::mem_fun(*this, &AnimationDialog::cmd_lower_node));
    xml_attribute_delete_button.signal_clicked().connect(sigc::mem_fun(*this, &AnimationDialog::cmd_delete_attr));
    set_attr.signal_clicked().connect(sigc::mem_fun(*this, &AnimationDialog::cmd_set_attr));
    attr_name.signal_changed().connect(sigc::mem_fun(*this, &AnimationDialog::onNameChanged));
    attr_value.signal_key_press_event().connect(sigc::mem_fun(*this, &AnimationDialog::sp_xml_tree_key_press), false);

    desktopChangeConn = deskTrack.connectDesktopChanged( sigc::mem_fun(*this, &AnimationDialog::set_tree_desktop) );
...
void AnimationDialog::setDesktop(SPDesktop *desktop)
{
 ...
 */
void AnimationDialog::tree_reset_context()
{
 ...
 */
void AnimationDialog::attr_reset_context(gint attr)
{
    if (attr == 0) {
        _message_context->set(Inkscape::NORMAL_MESSAGE,
...
bool AnimationDialog::sp_xml_tree_key_press(GdkEventKey *event)
{
...
void AnimationDialog::set_tree_desktop(SPDesktop *desktop)
{
    ...
    if (desktop) {
        sel_changed_connection = desktop->getSelection()->connectChanged(sigc::hide(sigc::mem_fun(this, &AnimationDialog::on_desktop_selection_changed)));
        document_replaced_connection = desktop->connectDocumentReplaced(sigc::mem_fun(this, &AnimationDialog::on_document_replaced));
...
void AnimationDialog::set_tree_document(SPDocument *document)
{
...
void AnimationDialog::set_tree_repr(Inkscape::XML::Node *repr)
{
...
void AnimationDialog::set_tree_select(Inkscape::XML::Node *repr)
{
    if (selected_repr) {
    ...
    if (repr) {
        ...
        if (sp_xmlview_tree_get_repr_node(SP_XMLVIEW_TREE(tree), repr, &node)) {
            ...
            g_message("AnimationDialog::set_tree_select : Couldnt find repr node");
        }
    } else {
        GtkTreeSelection *selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(tree));
        gtk_tree_selection_unselect_all (selection);
...
void AnimationDialog::propagate_tree_select(Inkscape::XML::Node *repr)
{
...
Inkscape::XML::Node *AnimationDialog::get_dt_select()
{
    if (!current_desktop) {
        return NULL;
    }
...
void AnimationDialog::set_dt_select(Inkscape::XML::Node *repr)
{
    if (!current_desktop) {
        return;
    }
...
/*void XmlTree::on_tree_select_row(GtkCTree *tree,
                                 ...
                                 gint column,
                                 gpointer data)*/
void AnimationDialog::on_tree_select_row(GtkTreeSelection *selection, gpointer data)
{
    AnimationDialog *self = static_cast<AnimationDialog *>(data);
...
void AnimationDialog::after_tree_move(SPXMLViewTree * /*attributes*/, gpointer value, gpointer data)
{
    AnimationDialog *self = static_cast<AnimationDialog *>(data);
...
void AnimationDialog::on_tree_select_row_enable(GtkTreeIter *node)
{
    if (!node) {
        return;
    }
...
gboolean AnimationDialog::xml_tree_node_mutable(GtkTreeIter *node)
{
...
void AnimationDialog::on_tree_unselect_row_disable()
{
    xml_text_new_button.set_sensitive(false);
    xml_element_new_button.set_sensitive(false);
    xml_node_delete_button.set_sensitive(false);
...
void AnimationDialog::on_tree_unselect_row_hide()
{
    attr_container.hide();
    text_container.hide();
}

void AnimationDialog::on_attr_select_row(GtkTreeSelection *selection, gpointer data)
{
    AnimationDialog *self = static_cast<AnimationDialog *>(data);
...
void AnimationDialog::on_attr_row_changed(SPXMLViewAttrList *attributes, const gchar * name, gpointer /*data*/)
{
...
void AnimationDialog::on_attr_unselect_row_clear_text()
{
    attr_name.set_text("");
    attr_value.get_buffer()->set_text("", 0);
}

void AnimationDialog::onNameChanged()
{
...
void AnimationDialog::onCreateNameChanged()
{
...
void AnimationDialog::on_desktop_selection_changed()
{
    if (!blocked++) {
...
void AnimationDialog::on_document_replaced(SPDesktop *dt, SPDocument *doc)
{
    if (current_desktop)
        sel_changed_connection.disconnect();

    sel_changed_connection = dt->getSelection()->connectChanged(sigc::hide(sigc::mem_fun(this, &AnimationDialog::on_desktop_selection_changed)));
    set_tree_document(doc);
}

void AnimationDialog::on_document_uri_set(gchar const * /*uri*/, SPDocument * /*document*/)
{
/*
...
gboolean AnimationDialog::quit_on_esc (GtkWidget *w, GdkEventKey *event, GObject */*tbl*/)
{
...
void AnimationDialog::cmd_new_element_node()
{
    ...
    name_entry->signal_changed().connect(sigc::mem_fun(*this, &AnimationDialog::onCreateNameChanged));
...
void AnimationDialog::cmd_new_text_node()
{
    g_assert(selected_repr != NULL);
...
void AnimationDialog::cmd_duplicate_node()
{
    g_assert(selected_repr != NULL);
...
void AnimationDialog::cmd_delete_node()
{
    g_assert(selected_repr != NULL);
...
void AnimationDialog::cmd_delete_attr()
{
    g_assert(selected_repr != NULL);
...
void AnimationDialog::cmd_set_attr()
{
    g_assert(selected_repr != NULL);
...
void AnimationDialog::cmd_raise_node()
{
    g_assert(selected_repr != NULL);
...
void AnimationDialog::cmd_lower_node()
{
    g_assert(selected_repr != NULL);
...
void AnimationDialog::cmd_indent_node()
{
...
void AnimationDialog::cmd_unindent_node()
{
    Inkscape::XML::Node *repr = selected_repr;
    g_assert(repr != NULL);
...
*/
bool AnimationDialog::in_dt_coordsys(SPObject const &item)
{
